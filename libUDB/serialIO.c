...
void udb_init_USART3(int16_callback_fptr_t tx_fptr, callback_uint8_fptr_t rx_fptr)
{
	...
	serial3_callback_received_byte = rx_fptr;
	...
	// Load all values in for U3STA SFR
	U3STAbits.UTXISEL1 = 0;     //Bit15 Int when Char is transferred (1/2 config!)
	U3STAbits.UTXINV = 1;       //Bit14 FrSky needs inverted output
	U3STAbits.UTXISEL0 = 1;     //Bit13 Other half of Bit15
	//                          //Bit12
	U3STAbits.UTXBRK = 0;       //Bit11 Disabled
	//U3STAbits.UTXEN = 1;        //Bit10 TX pins controlled by periph (handled below)
...
void __attribute__((__interrupt__, __no_auto_psv__)) _U3RXInterrupt(void)
{
	...
	{
		uint8_t rxchar = U3RXREG;
//		udb_serial_callback_received_byte(rxchar);
		if (serial3_callback_received_byte)
		{
			serial3_callback_received_byte(rxchar);
		}
	}
	U3STAbits.OERR = 0;
	interrupt_restore_corcon;
